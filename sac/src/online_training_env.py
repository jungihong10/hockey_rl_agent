...
class HockeyEnv(gym.Env, EzPickle):
  ...
  def _set_obs(self, observation):
    """Sets the environment state from the given observation array."""
    index = 0  # Track position in the observation array

    # Player 1
    pos_x = observation[0] + CENTER_X
    pos_y = observation[1] + CENTER_Y
    angle = observation[2]
    lin_vel_x = observation[3]
    lin_vel_y = observation[4]
    ang_vel = observation[5]

    self.player1.transform = ((pos_x, pos_y), angle)  # Move player1 properly
    self.player1.linearVelocity = (lin_vel_x, lin_vel_y)
    self.player1.angularVelocity = ang_vel

    # Player 2
    pos_x = observation[6] + CENTER_X
    pos_y = observation[7] + CENTER_Y
    angle = observation[8]
    lin_vel_x = observation[9]
    lin_vel_y = observation[10]
    ang_vel = observation[11]

    self.player2.transform = ((pos_x, pos_y), angle)  # Move player2 properly
    self.player2.linearVelocity = (lin_vel_x, lin_vel_y)
    self.player2.angularVelocity = ang_vel

    # Puck
    pos_x = observation[12] + CENTER_X
    pos_y = observation[13] + CENTER_Y
    lin_vel_x = observation[14]
    lin_vel_y = observation[15]

    self.puck.transform = ((pos_x, pos_y), self.puck.angle)  # Move puck properly
    self.puck.linearVelocity = (lin_vel_x, lin_vel_y)

    # Handle optional keep_mode values
    if self.keep_mode:
        self.player1_has_puck = observation[16]
        index += 1
        self.player2_has_puck = observation[17]
